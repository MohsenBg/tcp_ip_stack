...
node_loopback_callback_handler(param_t *param,
    ...
    int cmd_code = EXTRACT_CMD_CODE(
            tlv_buf);               /*Extract the cmdcode. If you did not specify the command during command tree building, default cmdcode would come as -1 */
    printf("cmd_code = %d\n", cmd_code);
    ...
    TLV_LOOP_BEGIN(tlv_buf, tlv) {
            if (strncmp(tlv->leaf_id, "node_name", strlen("node_name")) ==
                0) {   /*node-name is the leaf id which was specified in init_param()*/
                node_name = tlv->value;                                         /*actual value of leaf is present in tlv->value*/
            } else if (strncmp(tlv->leaf_id, "lo_address", strlen("lo-address")) ==
                       0) {    /*lo-address is the leaf id*/
    ...
    switch (cmd_code) {
        ...
        case CMDODE_CONFIG_NODE_LOOPBACK:
            printf("node_name = %s. lo-address = %s, op_code = %s\n",
                   node_name, lo_address,
...
main(int argc, char **argv) {
    ...
    /*Implementing CMD1*/
    {
...

        {
            ...
            libcli_register_param(&node,
                                  &node_name);   /*Add node_name leaf param as suboption of node param. Note that: show --> node --> node_name has been chained*/
...

            {
...

                {
                    ...
                    libcli_register_param(&loopback,
                                          &loopback_address);   /*Add node_name leaf param as suboption of <node-name> param. Note that: show --> node --> node_name --> lo-address has been chained*/
...

    {
...

        {
            ...
            init_param(&node_name,      /*Address of the current param*/
                       ...
                       "Help : Node name");  /*Help String*/
            libcli_register_param(&node,
                                  &node_name);   /*Add node_name leaf param as suboption of node param. Note that: config --> node --> node_name has been chained*/
...

            {
...

                {
                    ...
                    init_param(&loopback_address,      /*Address of the current param*/
                               ...
                               "Help : Node's loopback address");  /*Help String*/
                    libcli_register_param(&loopback,
                                          &loopback_address);   /*Add node_name leaf param as suboption of <node-name> param. Note that: config --> node --> node_name --> loopback --> <lo-address> has been chained*/
                    /* The below API should be called for param at which the command is supposed to invoke application callback rouine. 
                     * This CMDODE_SHOW_NODE_LOOPBACK code is sent to application using which we find which command was triggered, and accordingly what 
                     * are expected leaf params we need to parse. More on this ater.*/
