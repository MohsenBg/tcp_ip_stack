...
#include <signal.h>
#include "CommandParser/cmd_hier.h"
        ...
        if (param->options[i]) {
            ...
            if (IS_PARAM_CMD(param->options[i]) || IS_PARAM_NO_CMD(param->options[i])) {
                printf(ANSI_COLOR_MAGENTA "nxt cmd  -> %-31s   |   %s\n" ANSI_COLOR_RESET,
                       GET_CMD_NAME(param->options[i]), GET_PARAM_HELP_STRING(param->options[i]));
                continue;
            }
            printf(ANSI_COLOR_CYAN "nxt leaf -> %-32s  |   %s\n" ANSI_COLOR_RESET, GET_LEAF_TYPE_STR(param->options[i]),
                   GET_PARAM_HELP_STRING(param->options[i]));
            continue;
        }
        break;
    }
...
show_help_handler(param_t *param, ser_buff_t *b, op_mode enable_or_disable) {
    ...
    printf(ANSI_COLOR_YELLOW "                      This CLI Library was crafted by Abhishek Sagar.\n\n" ANSI_COLOR_RESET);
    printf(ANSI_COLOR_YELLOW "                      I have only made modifications to tailor it to my needs \n" ANSI_COLOR_RESET);
    return 0;
}
